...
export const UserDashboard = () => {
  ...
  const handleQueueUpdate = (update) => {
    ...
    if (update.status === 'serving') {
      toast.success(`It's your turn! Queue #${update.queueNumber} is now being served.`);
    }
  };
  ...
  const fetchBookings = async () => {
    try {
      ...
      // Join socket rooms for each active booking
      data.forEach(booking => {
        if (booking.status !== 'completed') {
          socket.emit('joinQueue', booking._id);
        }
      });
    } catch (error) {
                  ...
                  <div>
                    ...
                    {booking.status === 'pending' && (
                      <p className="text-sm text-gray-600 mt-2">
                        Current Position: #{booking.position}
                      </p>
                    )}
                  </div>
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm capitalize">
                      {booking.status === 'serving' ? 'Now Serving' : booking.status}
                    </span>
                  </div>
                </div>
              </div>
